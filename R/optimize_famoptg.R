...
#'
#' **Why Random Restart only?** [prep_famoptg()] enforces the p-rep
...
#'
#' **Constraint preservation**: every candidate is produced by [prep_famoptg()],
...
#'   Passed directly to [prep_famoptg()]. See that function for full
#'   documentation.
#'
...
#'   Passed directly to [evaluate_famoptg_efficiency()]. See that function
...
#'   Each restart calls [prep_famoptg()] with a new seed and evaluates the
...
#'   Default `TRUE`.
#'
#' @return The return value of [prep_famoptg()] for the best valid design
#'   found, augmented with two additional components:
#' \describe{
#'   \item{`efficiency`}{The full output of [evaluate_famoptg_efficiency()]
...
#' 369-382.
#'
#' @seealso [prep_famoptg()] for single-design construction without
#'   optimisation.
#'   [evaluate_famoptg_efficiency()] for standalone criterion evaluation.
#'   [optimize_alpha_rc()] for the equivalent optimizer for
#'   [alpha_rc_stream()] designs (also supports SA and GA methods).
#'
#' @examples
...
#' @export
#' 
optimize_famoptg <- function(

    # -- Construction arguments (forwarded to prep_famoptg) -------------------
    check_treatments,
    check_families,
    p_rep_treatments,
    p_rep_reps,
    ...
    # -- Evaluation arguments (forwarded to evaluate_famoptg_efficiency) ------
  ...
  .check_famoptg_integrity <- function(design) {
    ...
    if (nrow(non_check_fb) > 0) {
      ...
      if (any(counts > 1)) {
        warning(paste0(
          "[optimize_famoptg] Integrity check failed: ",
    ...
    if (length(p_rep_treatments) > 0) {
      for (i in seq_along(p_rep_treatments)) {
        ...
        if (actual != expected) {
          warning(paste0(
            "[optimize_famoptg] Integrity check failed: p-rep treatment '",
    ...
    if (length(unreplicated_treatments) > 0) {
      for (trt_u in unreplicated_treatments) {
        ...
        if (actual != 1L) {
          warning(paste0(
            "[optimize_famoptg] Integrity check failed: unreplicated treatment '",
    ...
    for (chk in check_treatments) {
      ...
      if (length(missing_blocks) > 0) {
        warning(paste0(
          "[optimize_famoptg] Integrity check failed: check treatment '",
  ...
  .emergency_fallback <- function() {
    warning(paste0(
      "[optimize_famoptg] No valid optimised design found. ",
    ...
    for (attempt in seq_len(10L)) {
      ...
      if (!is.null(fallback) && !is.na(fallback$score)) {
        warning(paste0(
          "[optimize_famoptg] Emergency fallback succeeded on attempt ", attempt,
    ...
    stop(paste0(
      "[optimize_famoptg] Emergency fallback failed after 10 attempts. ",
  ...
  run_pipeline <- function(s) {
    ...
    design <- tryCatch(do.call(prep_famoptg, args), error = function(e) NULL)
    ...
    eff   <- tryCatch(do.call(evaluate_famoptg_efficiency, ea), error = function(e) NULL)
  ...
  .update_best <- function(best, best_score, cand) {
    ...
    if (!.check_famoptg_integrity(cand$design)) {
      warning(paste0(
        "[optimize_famoptg] Candidate passed initial integrity check but ",
  ...
  if (verbose_opt) message(sprintf(
    "\n[optimize_famoptg] criterion = '%s' | n_restarts = %d | master seed = %d\n",
  ...
  if (n_failed > 0L) {
    ...
    msg  <- sprintf(
      "[optimize_famoptg] %d/%d restarts failed construction or integrity check (%.0f%%).",
  ...
  if (!is.null(best) && !is.null(best$design)) {
    if (!.check_famoptg_integrity(best$design)) {
      warning(paste0(
        "[optimize_famoptg] Stored best design failed final integrity check. ",
  ...
  if (verbose_opt) {
    ...
    message(sprintf(
      "\n[optimize_famoptg] Done. Best %s = %.6f (%s) | valid restarts = %d/%d\n",
