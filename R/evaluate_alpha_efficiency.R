...
#'
#' @description
#' `evaluate_alpha_efficiency()` takes a `field_book` produced by
#' [alpha_rc_stream()] and computes optimality criteria for the design under
#' a user-specified mixed model. It is fully decoupled from design
...
#'
#' Metrics are computed from the submatrix of \eqn{C^{-1}} corresponding to
#' treatment columns.
#'
...
#'
#' @param field_book Data frame produced by [alpha_rc_stream()]. Must contain
#'   columns: `Plot`, `Row`, `Column`, `Rep`, `IBlock`, `BlockInRep`,
#'   `Treatment`, `Check`.
#'
...
#'   (must match those used in [alpha_rc_stream()]).
#'
...
#'
#' @seealso [alpha_rc_stream()] to construct the design.
#'   [optimize_alpha_rc()] to search for a criterion-optimal design.
#'
#' @examples
...
#' @export
#' 
evaluate_alpha_efficiency <- function(
    field_book,
    n_rows,
    n_cols,
    check_treatments,
