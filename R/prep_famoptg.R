...
#'
#' @description
#' `prep_famoptg()` builds a repeated-check block design for plant breeding
...
#' CDmean optimality criteria call [evaluate_famoptg_efficiency()] on the
#' returned `field_book`. To search for an optimised design across many
#' randomisations call [optimize_famoptg()].
#'
#' @details
...
#' }
#'
#' @seealso [evaluate_famoptg_efficiency()] to compute A, D, and CDmean
#'   optimality criteria on the returned `field_book`.
#'   [optimize_famoptg()] to search for a criterion-optimal design using
#'   Random Restart.
#'
#' @examples
#' ## Augmented repeated-check design: checks repeated, entries unreplicated
#' design_aug <- prep_famoptg(
...
prep_famoptg <- function(
  ...
  # 5. BLOCK ASSIGNMENT
  # ============================================================
  # P-rep treatments: assign to p_rep_reps[i] distinct blocks each,
  # drawn without replacement to guarantee no treatment appears twice
  # in the same block.
  ...
  # Unreplicated treatments: distribute evenly across blocks
  ...
  place_checks <- function(checks, others, mode) {
    ...
    # systematic: space checks evenly
  ...
  build_blocks_once <- function(placement_mode) {
    ...
    for (b in seq_len(n_blocks)) {
      ...
      # Within-block shuffle to minimise same-group adjacency
      best     <- block_trt
  ...
  if (check_placement != "optimal") {
    ...
    # Optimal: maximise mean nearest-neighbour distance between checks
    best_score  <- -Inf
    best_blocks <- NULL
  ...
  if (isTRUE(use_dispersion)) {
    ...
    # Reuse shared helpers from alpha_rc_helpers.R
    ...
    if (length(movable) >= 2L && dispersion_iters > 0L) {
      ...
      pairs     <- .build_neighbor_pairs(final_data$Row, final_data$Column, radius = dispersion_radius)
      best_trt  <- trt
