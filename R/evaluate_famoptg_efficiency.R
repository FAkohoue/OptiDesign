...
#'
#' @description
#' `evaluate_famoptg_efficiency()` takes a `field_book` produced by
...
#'
#' @param field_book Data frame produced by [prep_famoptg()]. Must contain
...
#'   (must match those used in [prep_famoptg()]).
#'
...
#' \describe{
#'   \item{`"IID"`}{\eqn{G^{-1}_\text{entry} = \sigma_g^{-2} I}.}
...
#'     blocking level in [prep_famoptg()] designs (no replicate or
#'     incomplete-block nesting). This differs from `evaluate_alpha_efficiency()`
...
#'
#' @seealso [prep_famoptg()] to construct the design.
...
#' @export
#' 
evaluate_famoptg_efficiency <- function(
    field_book,
    n_rows,
    n_cols,
    check_treatments,
